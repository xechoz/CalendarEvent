...
BarWidget {
  ...
  // 写回必须命中真实 shell.json 条目(布局里实际生效的 id),不要硬编码。
  function entryId() {
  ...
  IpcHandler {
    ...
    function eventsDebug(): string {
      ...
      if (typeof p.debugInfo !== "function") return "no-debug-info"
      return p.debugInfo()
    }
  ...
  WidgetButton {
    ...
    onPressed: function(b) {
      if (b === Qt.RightButton) root.cycleFormat()
        ...
        if (root.bar && typeof root.bar.run === "function") root.bar.run("omarchy-menu-timezone")
      }
