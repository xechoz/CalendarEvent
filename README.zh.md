...
Omarchy bar 时钟 + 月历 + **事件日历**(带待办/提醒)插件,可在
[Omarchy 插件市场](https://plugins.omarchy.org/)安装:
...
<p>
  <img src="preview.png" alt="Calendar Event 面板" width="320">
  <img src="screenshot-detail.png" alt="当日事件视图" width="320">
</p>
...
市面上的日历类插件大致两类:**只读集成**(Google Calendar / CalDAV / agenda
等查看器 —— 只展示外部事件,不能本地创建、管理或提醒)或**纯农历显示**(不含
事件)。Calendar Event 是唯一"本地一体化"的:

- bar 时钟 + 月格 + 每日事件管理(全天 / 定时 / 多日 / 重复,支持按天跳过)—— 无需任何外部账号
- 待办 → 进行中 → 已完成 三态,列表行首圆点一键切换
- 桌面提醒:事件级提前量(5–60 分钟档位)、自动去重
- 中国法定节假日:内置 2026 全年,后续年份公布后自动联网获取
- 中 / 英界面,跟随系统语言

所有数据存于 `~/.local/share/omarchy-calendar`,完全离线可用。
